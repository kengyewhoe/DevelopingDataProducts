...
# Adds support for graphical tooltips and popovers, in order to enrich the UI.
library(shinyBS) 
    ...
    navbarPage(
        ...
        tabPanel(
            ...
            sidebarPanel(
                ...
                bsTooltip(id = "vs",
                          title = "Engine cylinder configuration (V-shape or straight)", 
                ...
                radioButtons("am",
                             ...
                             selected = 1, inline = TRUE),
                bsTooltip(id = "am",
                          title = "Transmission type (automatic or manual)",
                          placement = "right", options = list(container = "body")),
                
                sliderInput("cyl", "Cylinders",
                            min = minCylinders, max = maxCylinders,
                            value = defaultCar$cyl, step = 2),
                bsTooltip(id = "cyl",
                ...
                sliderInput("disp", "Displacement", 
                            ...
                            value = defaultCar$disp, step = 1),
                bsTooltip(id = "disp", 
                          title = "Engine displacement in cube inch",
                          placement = "right", options = list(container = "body")),
                
                sliderInput("hp", "Power",
                            min = minHp, max = maxHp,
                            value = defaultCar$hp, step = 1),
                bsTooltip(id = "hp", 
                          title = "Engine gross horsepower in HP",
                          placement = "right", options = list(container = "body")),
                
                sliderInput("drat", "Rear axle ratio",
                            min = minDrat, max = maxDrat, 
                            value = defaultCar$drat, step = 0.01),
                bsTooltip(id = "drat", 
                          title = "Rear axle ratio", 
                          placement = "right", options = list(container = "body")),
                
                sliderInput("wt", "Weight",
                            min = minWt, max = maxWt, 
                            value = defaultCar$wt, step = 1),
                bsTooltip(id = "wt", 
                          title = "Weight in 1000 lbs", 
                          placement = "right", options = list(container = "body")),
                
                sliderInput("qsec", "1/4 mile time", 
                            min = minQsec, max = maxQsec,
                            value = defaultCar$qsec, step = 0.1),
                bsTooltip(id = "qsec",
                          title = "1/4 mile time in seconds",
                          placement = "right", options = list(container = "body")),
                
                sliderInput("gear", "Gears"
                            , min = minGear, max = maxGear, 
                            value = defaultCar$gear, step = 1),
                bsTooltip(id = "gear",
                          title = "Number of forward gears in the transmission",
                          placement = "right", options = list(container = "body")),
                
                sliderInput("carb", "Carburetors",
                            min = minCarb, max = maxCarb,
                            value = defaultCar$carb, step = 1),
                bsTooltip(id = "carb",
        ...
        tabPanel(
            ...
            p("In this Shiny application we want to predict the mpg of a car based on a specific car dataset. By changing some characteristics of the car,
                          through the various sliders in the application and through a Random Forest model, the application predicts the value of the mpg"),
            ...
            tags$div("Here the codebook dataset:",
                     tags$ul(
